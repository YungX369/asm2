var db =window.openDatabase("cellphons","1.0", "Cellphons", 20000);
...
if(account_id !=""){login_success();}
 ...
 function fetch_database(){
   db.transaction(function(tx){
     ...
     tx.executeSql(query, ["Vivo", "Description 01"], fetch_transaction_sucess("Category 01"),
     transaction_error);
     tx.executeSql(query, ["Sam Sung", "Description 02"], fetch_transaction_sucess("Category 02"),
     transaction_error);
     tx.executeSql(query, ["Iphone", "Description 03"], fetch_transaction_sucess("Category 03"),
     transaction_error);
   });
 }
    ...
    product_list.innerHTML += `
      <div class="col-6 col-sm-4 col-lg-3 mt-3 p-3 product">
        <div class="product-img">
          <img src="image/1.jpg"width="100%" alt="${product.name}" > 
        </div>
        ...
        <div class="text-end">
          <button onclick="add_to_cart(this.id)" id="${product.id}" class="btn btn-secondary btn-sm">Add to Cart</button>
        </div>
      </div>
    `;
  }
...
<a class="btn btn-outline-dark ms-3" href="profile.html" > ${email} </a>
...
function show_cart_list(products){
  ...
  <td>${total}</td>
  <td>
  <button class="btn btn-secondary btn-sm">Order</button></td>
  </tr>`;
}
